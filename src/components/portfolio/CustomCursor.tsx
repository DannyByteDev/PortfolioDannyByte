  ...
  return (
    <>
      <motion.div
        ...
        animate={{ width: isHovering ? 64 : 32, height: isHovering ? 64 : 32, opacity: isVisible ? 1 : 0 }}
        transition={{ type: 'spring', stiffness: 300, damping: 20 }}
      >
        <div className="w-full h-full rounded-full border-2 border-cyan-400 flex items-center justify-center">
          {cursorText && (
            <motion.span initial={{ opacity: 0, scale: 0.5 }} animate={{ opacity: 1, scale: 1 }} className="text-[10px] font-bold text-cyan-400">
              {cursorText}
            </motion.span>
          )}
        </div>
      ...
      <motion.div
        ...
        style={{ x: cursorX, y: cursorY }}
        animate={{ width: isHovering ? 80 : 48, height: isHovering ? 80 : 48, opacity: isVisible ? 0.15 : 0 }}
